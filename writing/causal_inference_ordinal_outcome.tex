...
This paper begins from a classical observation in the ordinal response literature: ordinal outcomes identify latent treatment effects only up to location and scale transformation of the underlying latent scale. Consequently, treatment effects defined through arbitrary numerical codings of ordinal responses generally lack a clear identification justification without additional measurement assumptions \citep{Schroder2017a, Bond2018a, Bloem2022a} that the categories actually carry numerical meanings. Motivated by this identification limitation, I focus instead on distributional causal estimands defined directly on the observed ordinal outcome, including category-specific and cumulative treatment effects. These estimands respect the ordinal nature of the data and remain invariant to transformations of the latent scale.

Estimating these distributional causal effects requires modeling the conditional distribution of ordinal responses. Existing practice overwhelmingly relies on ordered logit and ordered probit models, which estimate the response distribution through a latent-index model with a fixed parametric error distribution. These models are attractive because they efficiently exploit the ordering of response categories and often perform well when their assumptions are approximately satisfied. In many survey experiments, however, these assumptions may be difficult to justify. Political attitudes are frequently polarized or clustered, producing multimodal latent distributions, and persuasive treatments often affect moderate respondents differently from strong supporters, generating nonlinear treatment-response relationships. In such settings, restrictive assumptions about the latent error distribution and index specification can distort estimated category probabilities and cumulative treatment effects, even when the experimental design itself identifies the causal effect. The practical challenge is therefore not causal identification, but flexible estimation of the ordinal response distribution under empirically realistic response mechanisms.

This paper develops a flexible likelihood-based framework for estimating causal effects with ordinal outcomes that relaxes these distributional assumptions while preserving the likelihood-based structure of classical ordered response models. The proposed estimator employs conditional normalizing flows to flexibly model the conditional distribution underlying observed ordinal responses without specifying a parametric latent error distribution. The same framework accommodates both structured latent-index specifications and fully nonparametric conditional distribution models, allowing researchers to balance interpretability and modeling flexibility within a unified estimation procedure. The resulting estimator directly recovers distributional causal estimands while remaining compatible with familiar latent-variable representations when such structure is scientifically appropriate.

I evaluate the proposed framework through Monte Carlo simulations and empirical replications of two published survey experiments. The simulations demonstrate that the proposed estimator maintains low bias across a broad range of latent error distributions and nonlinear response mechanisms under which conventional ordered response models perform poorly. I then revisit the survey experiments of \citet{Tomz2020a} and \citet{Mattingly2025a}. The empirical analyses show that flexible estimation of the ordinal response distribution can materially alter substantive conclusions regarding the magnitude and distribution of treatment effects.

The remainder of the paper proceeds as follows. Section~\ref{sec:causal_ordinal} formalizes causal inference with ordinal outcomes and motivates distributional causal estimands. Section~\ref{sec:estimation} reviews existing estimation approaches and introduces the proposed likelihood-based framework. Section~\ref{sec:simulation} evaluates finite-sample performance through Monte Carlo simulations. Section~\ref{sec:application} presents the empirical applications. Section~\ref{sec:conclusion} concludes.
...
Following the classical ordered response framework, let $Y_i^\ast$ denote respondent $i$’s latent attitude and let $Y_i \in \{0,1,\ldots,J\}$ denote the observed ordinal response. We assume that the observed response is generated through a threshold-crossing mechanism,
...
\node at (0,2.2) {\textbf{Latent Scale A}};
...
\node at (-3,1.7) {\small SD};
\node at (-1.2,1.7) {\small D};
\node at (0.3,1.7) {\small N};
\node at (1.8,1.7) {\small A};
\node at (3.3,1.7) {\small SA};

%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Panel B
%%%%%%%%%%%%%%%%%%%%%%%%%%

\node at (0,-0.2) {\textbf{Latent Scale B (Affine Transformation)}};

\draw[->] (-4,-1.2) -- (4,-1.2);

\foreach \x in {-3,-1,1.5,3}
    \draw (\x,-1.4)--(\x,-1.0);

\node at (-3.5,-0.7) {\small SD};
\node at (-2,-0.7) {\small D};
\node at (0.2,-0.7) {\small N};
\node at (2.2,-0.7) {\small A};
\node at (3.6,-0.7) {\small SA};

\end{tikzpicture}

\caption{Different latent scales and thresholds can generate the same observed ordinal responses. Consequently, the absolute location and scale of the latent attitude are not identified from ordinal data alone.}
\label{fig:identification}
\end{figure}

In this paper, the threshold-crossing representation serves as a measurement model linking latent attitudes to observed ordinal responses. It formalizes the idea that observed response categories preserve the ordering of respondents’ attitudes while discarding information about the distances between adjacent categories. Consequently, many different latent scales can induce exactly the same observed ordinal responses. Figure~\ref{fig:identification} illustrates this idea. Although the two panels correspond to different latent scales and threshold values, they generate the same observed response categories. The observed data therefore preserve the ordering of latent attitudes but not their absolute scale.
...
denote the corresponding observed ordinal potential outcome. I adopt the standard potential-outcomes framework and assume the usual conditions of SUTVA, conditional ignorability, and positivity \citep{Rubin1974a}. If the latent outcome $Y_i^\ast$ were directly observed, these assumptions would suffice to identify conventional causal estimands such as the average treatment effect on the latent attitude,

\[
ATE^\ast = \mathbb{E}\left[Y_i^\ast(1)-Y_i^\ast(0)\right].
\]

In practice, however, researchers observe only the ordinal response $Y_i$. The central challenge is therefore not whether treatment effects are causally identified, but whether the observed ordinal responses contain sufficient information to identify treatment effects on the underlying latent attitude. The next subsection revisits the classical identification result for ordered response models and shows how it motivates alternative causal estimands defined directly on the observed ordinal responses.

\subsection{Identification of Causal Effects from Ordinal Outcomes}
\label{subsec:identification}

The threshold-crossing framework provides a convenient representation of ordinal outcomes, but it also reveals a fundamental limitation of what can be learned from ordinal data. A classical result in the ordered response literature is that the latent-variable model is identified only up to location and scale. Although this normalization is routine in econometrics and psychometrics, its implications for causal inference are rarely made explicit in applied political science. In particular, survey experiments frequently interpret treatment effects estimated from numerically coded ordinal outcomes as effects on an underlying latent attitude without discussing the measurement assumptions required for such an interpretation.

To formalize this point, suppose the latent potential outcome satisfies the partially linear structural equation

\[
Y_i^\ast = h(X_i,\beta) + D_i^\top\tau + \varepsilon_i,
\]

where $h(X_i,\beta)$ is an unknown systematic component, $\tau$ denotes the treatment effect on the latent scale, and $\varepsilon_i$ has cumulative distribution function $F$. Under the threshold-crossing measurement model introduced above, the probability of observing category $j$ is

\[
\begin{aligned}
P(Y_i=j\mid X_i,D_i)
&=
F\left( \alpha_j - h(X_i,\beta) - D_i^\top\tau \right) \\
&\qquad -
F\left( \alpha_{j-1} - h(X_i,\beta) - D_i^\top\tau \right).
\end{aligned}
\]

Now consider any constants $a\in\mathbb{R}$ and $c>0$, and define
...
The transformed latent variable satisfies

\[
\tilde Y_i^\ast = h(X_i,\tilde\beta) + D_i^\top\tilde\tau + \tilde\varepsilon_i,
\]

where, for linear $h$, $\tilde\beta=c\beta$, $\tilde\tau=c\tau$, and $\tilde\varepsilon_i=c\varepsilon_i$. Because the thresholds and latent scale are transformed together, the induced probabilities of the observed ordinal responses remain unchanged.
...
Theorem~1 is not a failure of the experimental or quasi-experimental design. Under the standard assumptions of SUTVA, conditional ignorability, and positivity, causal effects remain identified for any well-defined outcome. Rather, the theorem highlights a measurement problem: ordinal responses do not uniquely determine the cardinal scale of the underlying latent attitude. The challenge therefore lies not in the experimental design but in the relationship between the observed ordinal responses and the unobserved latent construct.

\begin{table}[ht]
\centering
\small
\begin{tabular}{p{4.2cm}p{5.4cm}p{3.5cm}}
\hline
\textbf{Causal Quantity} &
\textbf{Scientific Interpretation} &
\textbf{Requires Additional Measurement Assumptions?} \\
\hline
...
&
Average change in the underlying latent attitude.

&
Yes
\\[1em]

Cardinalized average treatment effect
$E[s(Y(1))-s(Y(0))]$

&
Average change after assigning numerical scores to ordinal categories.

&
Yes
\\[1em]

Category probability effect
$P(Y(1)=j)-P(Y(0)=j)$

&
Change in the probability of observing category $j$.

&
No
\\[1em]

Cumulative probability effect
$P(Y(1)\ge j)-P(Y(0)\ge j)$

&
Change in the probability of responding at or above category $j$.
...
\caption{Common causal quantities for ordinal outcomes. Distributional causal effects are defined directly on the observed ordinal responses and therefore do not require assumptions about the cardinal scale of the latent attitude.}
\label{tab:estimands}
\end{table}

The identification result has an important implication for empirical practice. Different analyses of ordinal outcomes target different causal quantities and therefore require different measurement assumptions. Table~\ref{tab:estimands} summarizes the most common estimands. Latent treatment effects require assumptions that identify a cardinal latent scale, while treatment effects computed from numerically coded outcomes depend on the chosen scoring rule. Distributional causal effects, by contrast, are defined directly on the observed ordinal responses and therefore do not require assumptions about the cardinal spacing of the latent attitude. The next subsection formalizes these estimands and discusses their substantive interpretation.


\subsection{Distributional Causal Estimands}
\label{subsec:estimands}

The fundamental object is the distribution of potential outcomes. Let
...
Under the standard assumptions of SUTVA, conditional ignorability, and positivity introduced in the previous subsection, these probabilities are identified from the observed data through standardization:

\[
\pi_j(d) = \mathbb{E}_X \left[ P\left(Y_i=j \mid D_i=d,X_i \right) \right]
\]

where the expectation is taken with respect to the covariate distribution in the target population. In randomized experiments, where treatment assignment is independent of the covariates by design, this expression simplifies to the observed proportion of respondents in category $j$ within each treatment arm. Consequently, the primary statistical task is to estimate the conditional response probabilities
\[
P(Y_i=j\mid D_i,X_i),
\]
which serve as the building blocks for all distributional causal estimands considered in this paper.

The corresponding category-specific treatment effect is

\[
\Delta_j = \pi_j(1)-\pi_j(0).
\]

These effects describe how an intervention redistributes respondents across the observed response categories. For example, in a survey experiment on support for military intervention, $\Delta_{\text{Strongly Agree}}$ measures how treatment changes the proportion of respondents expressing the strongest level of support.

In many political science applications, researchers are interested in whether respondents exceed a substantively meaningful threshold rather than in a single response category. This motivates cumulative treatment effects,

\[
\Delta_{\ge j} = P\left(Y_i(1)\ge j\right) - P\left(Y_i(0)\ge j\right),
\]

which quantify changes in the probability of responding at or above category $j$. For example, a researcher may wish to estimate whether an informational treatment increases the probability that respondents express at least moderate support for a policy. Unlike dichotomizing the outcome before estimation, cumulative treatment effects can be computed for every threshold after estimating the ordinal response distribution, allowing the data to retain information from all response categories.

Distributional causal effects also distinguish between substantively different treatment mechanisms that may appear similar under conventional analyses. Two treatments may produce identical changes in the average of numerically coded responses while affecting entirely different parts of the response distribution. One treatment may primarily move respondents from strong opposition to neutrality, whereas another may convert moderate supporters into strong supporters. Although these interventions have different political implications, they may be indistinguishable once ordinal responses are reduced to a single coded mean. Distributional estimands preserve these distinctions by describing how treatment changes the entire response distribution rather than a single numerical summary.

This perspective also clarifies the relationship between the proposed estimands and existing empirical practice. Assigning numerical scores to ordinal categories and estimating mean differences targets

\[
\mathbb{E} \left[s(Y_i(1))-s(Y_i(0))\right],
\]

where $s(\cdot)$ denotes the chosen scoring rule. The resulting estimand is well defined for a given coding rule but generally does not identify a treatment effect on the underlying latent attitude. Similarly, dichotomizing an ordinal outcome estimates a cumulative treatment effect for a single threshold while discarding information contained in the remaining response categories. By contrast, estimating the full ordinal response distribution makes both category-specific and cumulative treatment effects available simultaneously without requiring researchers to commit to a particular numerical coding or threshold before estimation.

The remainder of the paper therefore focuses on estimating the conditional distribution of ordinal responses. Once this distribution is recovered, category-specific effects, cumulative treatment effects, and other substantively meaningful summaries can all be obtained as post-estimation quantities.
...
$$
where $f_Z$ is the base density and $\delta T_\theta^{-1}$ is the Jacobian of the inverse transformation. The corresponding CDF $F_\theta$ can be obtained by integrating $f_\theta$. By choosing a sufficiently rich family for $T_\theta$, normalizing flows can approximate a wide range of error distributions while preserving tractable likelihoods and gradients.
